...
# Store booking if valid and ALL required fields are present
if (
    parsed_booking.status == ParserBookingStatus.VALID
    and validation_result["is_valid"]
    and parsed_booking.name       # ✅ Added: ensure name not None
    and parsed_booking.email      # ✅ Added: ensure email not None  
    and parsed_booking.date       # ✅ Already checking
    and parsed_booking.time       # ✅ Already checking
    and parsed_booking.interview_type  # ✅ Added: ensure interview_type not None
):
    try:
        # Validate interview type before conversion
        interview_type = parsed_booking.interview_type.lower()
        if interview_type not in [e.value for e in InterviewType]:
            interview_type = "general"  # Fallback to general
        ...
        booking = BookingModel(
            ...
            interview_type=InterviewType(interview_type),  # ✅ Now safely converted
